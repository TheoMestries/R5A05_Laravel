...
@section('content')
    <div class="relative min-h-screen bg-dots-darker bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
        <div class="container mx-auto p-6">
            ...
            @if($events->isEmpty())
                <p class="text-gray-600 dark:text-gray-400">Vous n'avez créé aucun événement pour le moment.</p>
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($events as $event)
                        <a href="{{ route('event.show', $event->id) }}" class="items-center grid bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden mb-4 md:mb-0">
                            @if($event->image)
